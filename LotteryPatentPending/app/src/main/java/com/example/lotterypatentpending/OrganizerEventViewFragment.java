...
public class OrganizerEventViewFragment extends Fragment {
    ...
    private TextView eventTitle, eventDescr, eventLocation, eventDate,
            eventRegStart, eventRegEnd, maxEntrants, waitListCap, eventTag;
    ...
    public void onViewCreated(@NonNull View v, @Nullable Bundle savedInstanceState) {
        ...
        eventViewModel.getEvent().observe(getViewLifecycleOwner(), event -> {
            ...
            // --- Capacity + waiting list calculations ---
            ...
            // Accepted / capacity
            String capacityText = "Accepted: " + acceptedCount + " / " + capacity;
            maxEntrants.setText(capacityText);

            // Waiting list size / capacity (or N/A)
            String waitListText;
            if (wlCap == -1) {
                waitListText = "N/A";
            } else {
            ...
            String dateText = (event.getDate() == null)
                    ? "TBD"
                    : DateTimeFormatHelper.formatTimestamp(event.getDate());

            String regStartText = (event.getRegStartDate() == null)
                    ? "TBD"
                    : DateTimeFormatHelper.formatTimestamp(event.getRegStartDate());

            String regEndText = (event.getRegEndDate() == null)
                    ? "TBD"
                    : DateTimeFormatHelper.formatTimestamp(event.getRegEndDate());
            ...
            if (event.getPosterBytes() != null && event.getPosterBytes().length > 0) {
                posterImage.setImageBitmap(
                        BitmapFactory.decodeByteArray(
                                event.getPosterBytes(), 0, event.getPosterBytes().length
                        )
                );
            }
        });
        ...
        geoLocationReq.setOnCheckedChangeListener(
                (bv, isChecked) -> eventViewModel.updateGeoRequired(isChecked)
        );
        ...
        viewMapBtn.setOnClickListener(view -> {
            if (currentEvent != null && currentEvent.isGeolocationRequired()) {
                ...
                Toast.makeText(requireContext(),
                        "Enable geo-location first.", Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void showNotificationOptionsDialog() {
        ...
        new androidx.appcompat.app.AlertDialog.Builder(requireContext())
                ...
                .setItems(new CharSequence[]{
                        ...
                        "All waitlisted entrants",     // ENTERED / NOT_SELECTED / etc.
    ...
    private void showComposeDialog(TargetGroup group) {
        View dialogView = LayoutInflater.from(getContext())
                .inflate(R.layout.dialog_notification_compose, null);
        ...
        dialog.setOnShowListener(d -> {
            Button sendBtn = dialog.getButton(
            ...
            sendBtn.setOnClickListener(v -> {
                ...
                if (group == TargetGroup.CHOSEN_SIGNUP) {
                    organizerNotifier.notifySelectedToSignUp(
                            currentOrganizerId, currentEvent.getId(), eventName, body);
                } else if (group == TargetGroup.WAITLIST) {
                    organizerNotifier.notifyEntrantsInWaitlist(
                            currentOrganizerId, currentEvent.getId(), eventName, body);
                } else if (group == TargetGroup.ATTENDING) {
                    organizerNotifier.notifyEntrantsAttending(
                            currentOrganizerId, currentEvent.getId(), eventName, body);
                } else if (group == TargetGroup.CANCELLED) {
                    organizerNotifier.notifyCancelledEntrants(
                            currentOrganizerId, currentEvent.getId(), eventName, body);
                }
